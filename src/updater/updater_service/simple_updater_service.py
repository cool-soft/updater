...
class SimpleUpdaterService(AbstractUpdaterService):
    ...
    async def join(self) -> None:
        updater_logger.debug("Waiting for service stop")
        await self._wait_service_running_state(self.ServiceRunningState.STOPPED)

    def is_running(self) -> bool:
        updater_logger.debug(f"Service running status is {self._running_state}")
        return self._running_state is not self.ServiceRunningState.STOPPED

    async def stop_service(self) -> None:
        updater_logger.debug("Stopping service")
    ...
    async def start_service(self) -> None:
        updater_logger.debug("Starting service")
        await self._set_service_running_state(self.ServiceRunningState.RUNNING)
        asyncio.create_task(self._run())

    async def _run(self) -> None:
        updater_logger.debug("Service is started")
        try:
            while True:
                updater_logger.debug("Run update cycle")
    ...
    async def _update_items(self) -> None:
        ...
        for item in unpacked_dependencies_graph:
            if self._is_need_update_item(item, update_start_datetime):
                updater_logger.debug(f"Updating item {item.__class__.__name__}")
                await item.update_async()

        updater_logger.debug("Items are updated")

    def _is_need_update_item(self,
                             item: AbstractUpdatableItem,
                             update_start_datetime: datetime
                             ) -> bool:
        updater_logger.debug("Check need update item")
        ...
        updater_logger.debug(f"Item need update status is {need_update}")
        return need_update
    ...
    def _item_update_datetime_has_come(self,
        ...
        updater_logger.debug(f"Check item update datetime has come {item.__class__.__name__}")
        ...
        updater_logger.debug(f"Item need update by datetime is {need_update}")
        return need_update

    def _is_item_dependencies_are_updated(self,
                                          item: AbstractUpdatableItem
                                          ) -> bool:
        updater_logger.debug(f"Check that item dependencies are updated for {item}")
        ...
        updater_logger.debug(f"Item dependencies update status is {dependencies_updated}")
    ...
    async def _wait_for_first_completed_coroutines(self,
        ...
        updater_logger.debug("Waiting for coroutines and cancel not done")
    ...
    async def _sleep_to_next_update(self) -> None:
        ...
        updater_logger.debug(f"Sleeping {timedelta_to_next_update}")
    ...
    def _get_unpacked_dependencies_graph(self,
        ...
        updater_logger.debug("Unpacked dependencies graph is requested")
        ...
        updater_logger.debug(f"Dependency count of {item.__class__.__name__}: {len(unpacked_graph)}")
